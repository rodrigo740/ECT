...
GraphBFSWithQueue* GraphBFSWithQueueExecute(Graph* g, unsigned int startVertex) {
  ...
  traversal->marked = (unsigned int*) malloc(numVertices* sizeof(unsigned int));
  ...
  QueueEnqueue(q,traversal->startVertex);
  ...
  while (!QueueIsEmpty(q))
  {
    ...
    for (int i = 1; i <= w[0]; i++)
    {
      if (traversal->marked[w[i]]==0)
      {
        QueueEnqueue(q,w[i]);
        traversal->marked[w[i]]=1;
        traversal->predecessor[w[i]] = v;
      }
    }
  }
  QueueDestroy(&q);
