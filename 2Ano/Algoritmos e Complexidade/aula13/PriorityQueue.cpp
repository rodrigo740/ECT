...
int _insertStartingFrom(PriorityQueue* ph, void* item, int n) {
   while (n > 0) {
      int p = _parent(n);
      
      // if item not less than _parent, then we've found the right spot!
      if (!(ph->compare(item,ph->array[p]))) break;
...
void PriorityQueueInsert(PriorityQueue* ph, void* item) {
   ...
   _insertStartingFrom(ph, item, ph->size);
   ph->size++;
...
int _findItem(PriorityQueue* ph, void* item) {
